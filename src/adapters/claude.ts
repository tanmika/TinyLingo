...
**触发词选择至关重要**：触发词必须是用户在日常对话中会自然说出的词，而不是技术概念名。
例如：用户会说"帮我编译"而不是"执行 gradle 构建"，会说"推送一下"而不是"执行 git push"。
选错触发词 = 记录永远不会被匹配到。

### 重复术语处理

如果 record 返回 \`Already exists\`，说明该术语已有记录。此时：

1. 对比新旧描述，判断是否需要更新
2. 如果新旧含义一致（如仅路径不同），提取公共描述后用 \`--force\` 更新
3. 如果含义不同，先告知用户已有记录及其内容，由用户决定是否覆盖
4. 不要未经思考直接 \`--force\` 覆盖
